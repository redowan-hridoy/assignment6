...
const loadVideos  = async(category_id) => {
    ...
    const data = await response.json();
    const cardContainerDetails = document.getElementById('card-container-details');
    cardContainerDetails.innerHTML = "";
    data.data.forEach((vlog) => {
        console.log(vlog);
        const div2 = document.createElement('div');
        div2.innerHTML = `
        <div class="card bg-base-100 shadow-xl mb-4">
        <div><figure><img class="h-56 w-80 rounded" src="${vlog.thumbnail}" alt="" /></figure></div>
        <section class="flex gap-4 p-2">
        <div><img class="h-12 w-12 rounded-full" src="${vlog?.authors[0]?.profile_picture}" alt=""></div>
        <h2 class="card-title">${vlog.title}</h2>
        </section>
        <div class="p-2 flex gap-3">
          <div><p>${vlog?.authors[0]?.profile_name}</p></div>
          <div><p><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
        ...
        <div class:"p-8"><p>${vlog?.others?.views} views</p></div>
      </div>
        `;
        cardContainerDetails.appendChild(div2);
    });
